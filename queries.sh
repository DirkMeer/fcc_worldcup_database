...
#first create a winners only table per game:
#SELECT * FROM games INNER JOIN teams ON games.winner_id = teams.team_id;
...
#first create appropriate join table with both winners and losers per match displayed.
#SELECT * FROM games INNER JOIN teams ON games.winner_id = teams.team_id OR games.opponent_id = teams.team_id;
...
echo "$($PSQL "SELECT DISTINCT(name) FROM games INNER JOIN teams ON games.winner_id = teams.team_id OR games.opponent_id = teams.team_id WHERE name LIKE 'Co%'")"
